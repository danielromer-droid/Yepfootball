...
function isAfterMidnightParis(date = new Date()) {
  // The daily snapshot represents YESTERDAY's completed matches.
  // Cron can run at 00:00, 00:15, 00:30, etc. The overnight window
  // also gives the Worker several hours to recover from a missed run.
  const p = parisParts(date);
...
function snapshotAgeHours(snapshot, now = new Date()) {
  if (!snapshot?.publishedAt) return Infinity;
  const published = new Date(snapshot.publishedAt).getTime();
  if (!Number.isFinite(published)) return Infinity;
  return Math.max(0, (now.getTime() - published) / 3600000);
}

async function scores(env) {
  const now = new Date();
  const targetDate = yesterdayParisDate(now);
  const cached = await readLatest(env);

  // A successful snapshot is deliberately kept in KV. Never erase it because
  // the upstream API is unavailable. During the normal daytime period, the
  // current successful snapshot remains visible.
  //
  // IMPORTANT: If KV is empty (for example after the Worker/KV was first
  // configured), initialise it immediately from YESTERDAY's completed
  // matches. This means the site does not stay empty until the next midnight.
  // The same bootstrap also recovers automatically if a Cron Trigger was missed.
  if (cached?.sourceDate === targetDate && cached?.publishedAt) {
    return {
      ...cached,
      cacheStatus: isAfterMidnightParis(now) ? "already-published" : "last-successful-snapshot",
      checked: now.toISOString(),
      snapshotAgeHours: Number(snapshotAgeHours(cached, now).toFixed(1))
    };
  }

  // If we are outside the overnight publication window and KV contains an
  // older successful snapshot, keep showing it rather than replacing it with
  // an error. We only fetch a new snapshot outside the window when KV is empty
  // or has never been initialised for this site.
  const needsBootstrap = !cached;
  if (!isAfterMidnightParis(now) && !needsBootstrap) {
    return {
      ...cached,
  ...
  try {
    const events = await afMatchesForDate(env, targetDate);
    const finished = events.filter(e => isCompleted(e.status));

    // Never replace a good previous snapshot with an empty/failed upstream
    // response. This is especially important just after midnight when APIs
    // may not yet have finalised every result.
    if (!finished.length) {
      if (cached) {
        return {
          ...cached,
          ...
          checked: now.toISOString(),
          warning: `No completed matches were returned for ${targetDate}; keeping the last successful snapshot.`
        };
      }
      throw new Error(`No completed matches were returned for ${targetDate}`);
    }

    const snapshot = buildSnapshot(events, targetDate, now);
    snapshot.cacheStatus = "fresh";
    snapshot.checked = now.toISOString();
    await writeLatest(env, snapshot);
    return snapshot;
  } catch (error) {
    // KV is the source of continuity: API errors must never delete or
    // overwrite the last successful result set.
    if (cached) {
      return {
        ...cached,
...
export default {
  ...
  async scheduled(event, env, ctx) {
    // Cron runs every 15 minutes. After midnight, scores() publishes the
    // previous day's completed results whenever the new snapshot is missing.
    // There is no dependency on an exact 23:50 execution time.
