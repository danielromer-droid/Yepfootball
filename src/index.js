...
async function scores(
  ...
  /*
     Scores cover TWO calendar days:
       - yesterday: completed results
       - today: live, finished and upcoming matches

     We query API-Football directly rather than returning the old
     LATEST_SCORES snapshot, because that snapshot only contains one day.
  */
  ...
  try {
    ...
    /* Short edge cache protects the API quota while keeping today's
       scores reasonably fresh. */
    const cacheUrl = new URL(request.url);
    cacheUrl.pathname = "/api/scores-v20260923";
    cacheUrl.search = `?yesterday=${yesterday}&today=${today}`;
      ...
      dates.map(async item => {
        try {
          ...
          return {
            ...
            fixtures: Array.isArray(result.data?.response)
    ...
    for (const result of results) {
      const relevantFixtures = result.fixtures.filter(
        item => SCORE_LEAGUE_IDS.has(Number(item?.league?.id))
      );

      for (const item of relevantFixtures) {
    ...
    const responseData = {
      ...
      finished,
      upcoming,
      yesterdayEvents: events.filter(e => e.scoreDay === "yesterday"),
      todayEvents: events.filter(e => e.scoreDay === "today"),
      count: events.length,
      liveCount: live.length,
      updated: new Date().toISOString(),
      scoreWindow: "yesterday-and-today"
    };
