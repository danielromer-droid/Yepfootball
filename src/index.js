...
function normaliseFootballDataFixture(
  ...
  return {
    ...
    leagueId:
      COMPETITIONS.find(
        c => c.footballDataCode === item.competition?.code
      )?.apiFootballId ??
      item.competition?.id ??
      null,
...
async function scores(
  ...
  /*
     Scores source:
       football-data.org is used for the latest completed results.

     Behaviour:
       1. Include today's live/paused matches and today's scheduled matches.
       2. Find the most recent completed match for each supported competition.
       3. Return every completed match from that latest date for that league.
       4. Search backwards in 10-day windows because football-data.org limits
          date ranges to 10 days per request.

     This makes the Scores page useful during international breaks as well.
  */

  try {
    const today = todayUTC();
    const cacheUrl = new URL(request.url);
    cacheUrl.pathname = "/api/scores-v20260923-fd-latest";
    cacheUrl.search = `?date=${today}`;

    const cache = caches.default;
    const cached = await cache.match(cacheUrl);
    if (cached) return cached;

    const competitionCodes = COMPETITIONS
    ...
    /* Search backwards in 10-day windows, stopping when every
       supported competition has at least one completed match. */
    for (let offset = 0; offset < 90; offset += 10) {
      const to = addDays(today, -offset);
      const from = addDays(today, -(offset + 9));

      const path =
        `/matches?competitions=${competitionCodes}` +
        `&dateFrom=${from}&dateTo=${to}`;

      try {
        const result = await footballData(path, env);
        ...
        diagnostics.push({
          ...
          apiCount: result.data?.resultSet?.count ?? result.data?.count ?? 0
        });

        for (const match of matches) {
          const code = match?.competition?.code;
          if (!COMPETITIONS.some(c => c.footballDataCode === code)) continue;

          const id = String(match?.id ?? `${match?.utcDate}-${code}-${match?.homeTeam?.name}-${match?.awayTeam?.name}`);
          matchesById.set(id, match);

          if (match?.status === "FINISHED") {
            const date = String(match?.utcDate || "").slice(0, 10);
            if (date) {
              const previous = latestDateByLeague.get(code);
              if (!previous || date > previous) {
                latestDateByLeague.set(code, date);
              }
            }
          }
        }

        if (latestDateByLeague.size === COMPETITIONS.length) {
          break;
        }
      } catch (error) {
        diagnostics.push({
    ...
    const normalised = Array.from(matchesById.values())
      .map(normaliseFootballDataFixture)
      .filter(match => SCORE_LEAGUE_IDS.has(Number(match.leagueId)));

    const finished = normalised.filter(match =>
      ["FINISHED"].includes(match.status)
    );

    const live = normalised.filter(match =>
      ["IN_PLAY", "PAUSED", "LIVE"].includes(match.status)
    );

    const todayUpcoming = normalised.filter(match => {
      const date = String(match.date || "").slice(0, 10);
      return date === today && ["TIMED", "SCHEDULED"].includes(match.status);
    });

    const latestAvailable = finished
      .filter(match => {
        const code = match?.leagueCode;
        const latestDate = COMPETITIONS.find(c => c.apiFootballId === Number(match?.leagueId))
          ? latestDateByLeague.get(code)
          : null;
        return latestDate && String(match.date || "").slice(0, 10) === latestDate;
      })
      .sort((a, b) => new Date(a.date || 0) - new Date(b.date || 0));

    /* Today's live/paused and scheduled matches are useful alongside
       the latest completed result for each competition. */
    const todayLive = live.filter(match =>
      String(match.date || "").slice(0, 10) === today
    );

    const eventsMap = new Map();
    for (const match of [...latestAvailable, ...todayLive, ...todayUpcoming]) {
      eventsMap.set(String(match.id), match);
    }
    ...
    const yesterdayEvents = events.filter(match =>
      String(match.date || "").slice(0, 10) === yesterday
    );
    const todayEvents = events.filter(match =>
      String(match.date || "").slice(0, 10) === today
    );
    ...
    const responseData = {
      ...
      todayEvents,
      latestAvailable,
      latestDateByLeague: Object.fromEntries(latestDateByLeague.entries()),
      count: events.length,
      liveCount: todayLive.length,
