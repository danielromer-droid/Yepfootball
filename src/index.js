...
async function scores(env) {
  ...
  // the upstream API is unavailable, and never overwrite it before the new
  // calendar day has started.
  if (!isAfterMidnightParis(now)) {
    if (cached) {
      return {
        ...cached,
  ...
  // Once yesterday's snapshot has been published, keep serving it for the
  // entire day. It remains in KV for 24+ hours until the next successful
  // overnight publication.
  if (cached?.sourceDate === targetDate && cached?.publishedAt) {
    return {
      ...cached,
      cacheStatus: "already-published",
      checked: now.toISOString(),
