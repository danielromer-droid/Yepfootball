    ...
    try {
      ...
      if (cached) {
        ...
        return json({
          ...
          live: cachedEvents.filter(e =>
            ["1H", "2H", "HT", "ET", "P"].includes(e?.status)
          ),
          finished: cachedEvents.filter(e =>
            ["FT", "AET", "PEN"].includes(e?.status)
          ),
          upcoming: cachedEvents.filter(e =>
            ["NS", "TBD"].includes(e?.status)
          ),
          liveCount: cachedEvents.filter(e =>
            ["1H", "2H", "HT", "ET", "P"].includes(e?.status)
          ).length,
    ...
    const events =
      relevantFixtures.map(
        item => {
          ...
          return normaliseApiFootballFixture(
            item,
            competition
          );
        }
