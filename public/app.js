...
function renderLatestScores(data, options = {}) {
  ...
  const snapshotDate = data.sourceDate || data.date;
  ...
  if (!events.length) {
    grid.innerHTML = `
      <div class="empty">
        ...
        <br>
        <small>The latest successful daily snapshot will remain visible until a new one is published at 23:50 CET.</small>
      </div>
    `;
  } else {
    const day = fmtDay(snapshotDate);
    const fallback =
      options.fallback ||
      data.cacheStatus === "fallback" ||
      data.cacheStatus === "last-successful-snapshot" ||
      data.cacheStatus === "previous-snapshot-retained";

    const note = fallback
      ? `<small>Last successfully published results · ${esc(fmtTime(data.publishedAt))}</small>`
      : `<small>Published ${esc(fmtTime(data.publishedAt))} CET/CEST</small>`;
...
async function loadScores() {
  ...
  try {
    ...
    if (status) {
      if (data.cacheStatus === "fallback") {
        status.textContent = `Last successful results · ${data.sourceDate || ""}`;
      } else if (data.cacheStatus === "last-successful-snapshot" ||
                 data.cacheStatus === "previous-snapshot-retained") {
        status.textContent = `Latest available results · ${data.sourceDate || ""}`;
      } else if (data.publishedAt) {
        status.textContent = `${count} results · Published ${fmtTime(data.publishedAt)} CET`;
      } else {
      ...
      if (data.sourceDate) {
        const suffix =
          data.cacheStatus === "fallback"
            ...
            : "";
        $("#updated").textContent =
          `Latest scores · ${data.sourceDate}${suffix}`;
      } else {
        $("#updated").textContent =
    ...
    // Prefer the server KV snapshot, then the browser's local copy.
    const fallback =
      latestScoresData ||
      readBrowserSnapshot("yepfootball_latest_scores");
    ...
    if (fallback?.events?.length) {
      ...
      if (status) {
        status.textContent =
          `Last successful results · ${fallback.sourceDate || fallback.date || ""}`;
      }

      if ($("#updated")) {
        $("#updated").textContent =
          `Latest scores · ${fallback.sourceDate || fallback.date || ""} · Last successful update`;
      }
    } else {
      if ($("#scores-grid")) {
        $("#scores-grid").innerHTML = `
          <div class="empty">
            ...
            <br>
            <small>The first daily snapshot is published at 23:50 CET.</small>
          </div>
        `;
      }
